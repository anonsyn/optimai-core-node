...
const Title = () => {
  const title = useAppSelector(headerSelectors.title)
  const backPath = useAppSelector(headerSelectors.backPath)
    ...
    <div className="flex w-full items-center gap-3">
      {title ? (
        <>
          ...
          <h1 className={'text-18 leading-normal font-semibold text-white'}>{title}</h1>
        </>
      ) : (
        <Logo className="h-9" />
      )}
